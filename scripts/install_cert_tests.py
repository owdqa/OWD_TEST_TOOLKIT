...
def main():
    ...
    tests_repo = 'git@pdihub.hi.inet:owd/owd-obcertification.git'
    os.chdir(tkdir)
    ...
    cloned = False
    if not os.path.exists('owd-obcertification'):
        subprocess32.call('git clone {} >> {} 2>&1'.format(tests_repo, logfile), shell=True)
        cloned = True
    print "Opening certification dir"

    with open(logfile, 'a') as f:
        os.chdir('owd-obcertification')
        f.write('Switching to branch {} of owd-obcertification'.format(branch))
        ...
        if not cloned:
            ...
            cmd = 'git ls-remote --heads {} {}'.format(tests_repo, branch)
            ...
            if local_commit != remote_commit:
                ...
                f.write('\n\nUpdating owd-obcertification repository...')
                f.write('\n=======================================\n')
