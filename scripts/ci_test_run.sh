...
sudo DEVICE=$DEVICE BRANCH=$BRANCH DEVICE_BUILDNAME=$DEVICE_BUILDNAME ON_CI_SERVER=$ON_CI_SERVER HTML_FILEDIR=$HTML_FILEDIR python $OWD_TEST_TOOLKIT_DIR/scripts/ffox_test_runner.py --testvars=$OWD_TEST_TOOLKIT_DIR/config/gaiatest_testvars.json --address=localhost:2828 tests/$TEST_TYPE
