...
var GaiaApps = {
  ...
  getRunningAppOrigin: function(name) {
    //NB: names should be already normalized
    // console.log("[getRunningAppOrigin] of app: " + name);
    
    // Get locales for later comparison with runningApps' name
    let apps = window.wrappedJSObject.Applications.installedApps;
    let locales;
    for (let manifestURL in apps) {
      let app = apps[manifestURL];

      // Getting locales taking into account whether app has entry_points
      let entry_points = app.manifest.entry_points;
      // console.log("[getRunningAppOrigin] Entry_points: " + entry_points);
      if ((entry_points) && (Object.keys(entry_points).length > 0)) {
        // console.log("[getRunningAppOrigin] Has entry points");
        for (let ep in entry_points) {
          let anEp = entry_points[ep];
          // console.log("[getRunningAppOrigin] Entry point name: " + anEp["name"]);

          if (anEp["name"] === name) {
            // console.log("[getRunningAppOrigin] Entry point locales: " + anEp["locales"]); 
            if (anEp["locales"]) {
              // console.log("[getRunningAppOrigin] Has locales");
              locales = anEp["locales"];
              break;
            }
          }
        }
        ...
        if (app.manifest.name === name) {
          for (let prop in app.manifest){
            if (prop === "locales"){
              locales = app.manifest[prop];
              break;
            }
          }
        }
    ...
    if (!!locales) {
      for (let property in runningApps) {
        for (let local in locales) {
          // console.log("[Name comparison][property] ->" + runningApps[property].name);
          // console.log("[Name comparison][locales]  ->" + locales[local]["name"]);
          if (runningApps[property].name == locales[local]["name"]) {
            // console.log(">>>>>>>>> Assignation of origin <<<<<<<<<<<");
            origin = property;
            break;
          }
        }
      ...
      console.log(">>> No locales presented -> No way to distinguish languages")
    }
    return origin;
  },
          ...
          window.addEventListener('apploadtime', function apploadtime() {
            ...
            waitFor(
              ...
              function() {
                ...
                let displayedApp = windowManager.getDisplayedApp();
                console.log(">>>> displayed app: " + displayedApp);
                return  displayedApp == origin;
              }
            );
          });
